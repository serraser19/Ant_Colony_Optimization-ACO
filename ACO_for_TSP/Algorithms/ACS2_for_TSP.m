...
%           ALGORITHM  : ANT COLONY SYSTEM (ACS2) FOR TSP
% ACS2 : Each ant updates all the edges of its respective path 
...
function sol = ACS2_for_TSP(x,y,Maxit,num_Ants,alpha,beta,rho,q0,phi,tau0)

[m,n] = size(x);   % n : total nº of graph verticeso
...
for iter=1:Maxit
    ...
    %            ------------ AS -------------            
        ...
        for i = vsit_vert(k,:)
            ...
            pher_mat(i,vsit_vert(k,posi+1)) = (1-rho)*pher_mat(i,vsit_vert(k,posi+1)) + rho*tau0;
            pher_mat(vsit_vert(k,posi+1),i) = pher_mat(i,vsit_vert(k,posi+1));   
        end
        % edge back to the starting point
        pher_mat(vsit_vert(k,n),vsit_vert(k,1)) = (1-rho)*pher_mat(vsit_vert(k,n),vsit_vert(k,1)) + rho*tau0;            
    ...
    for i = 1:n
         for j = i+1:n
            pher_mat(i,j) = (1-phi)*pher_mat(i,j) + phi*delta_best(i,j);            
            pher_mat(j,i) = pher_mat(i,j);   
         end
    end
