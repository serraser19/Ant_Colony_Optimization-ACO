...
%                   ALGORITHM  : ACO FOR CVRP2
%          CVRP2 : Building solutions with formula (4)
...
%            -->  lambda   : Influence of capacity (kappa)
%            -->  sigma    : Nº of elitists ants

% Output     -->  sol      : Optimal tour length
% """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

function sol = ACO_for_CVRP2(x,y,q,Q,Maxit,num_Ants,alpha,beta,rho,gamma,lambda,sigma)
...
while iter <= Maxit
    ...
    while k <= num_Ants          
        ...
        while pos < n-1  
            ...
            for j = nonvsit_vert  
                fprob(k,j) = (pher_mat(iaux,j)^alpha)*((1/len_mat(iaux,j))^beta)*(mu(iaux,j)^gamma)*(kappa(iaux,j)^lambda); % formula (4)
            end           
