...
%                   ALGORITHM  : ACO FOR CVRP3
%          CVRP3 : Building solutions with formula (3)
%                Pheromone trace update with formula (7)
% #########################################################################
...
%            -->  gamma    : Relative importance of saving
%            -->  sigma    : Nº of elitists ants

% Output     -->  sol      : Optimal tour length
% """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""

function sol = ACO_for_CVRP3(x,y,q,Q,Maxit,num_Ants,alpha,beta,rho,gamma,sigma)
...
% --------------- Parametric saving function -----------------
for i = 1:n
    for j = i+1:n
        mu(i,j) = len_mat(i,1)+len_mat(1,j)-len_mat(i,j)+abs(len_mat(i,1)-len_mat(1,j)); 
        mu(j,i) = mu(i,j);
    end
end
...
while iter <= Maxit
    ...
    while k <= num_Ants          
        ...
        while pos < n-1  
            ...
            for j = nonvsit_vert  
                fprob(k,j) = (pher_mat(iaux,j)^alpha)*((1/len_mat(iaux,j))^beta)*(mu(iaux,j)^gamma); % formula (3)
            end           
    ...
    if s1<num_Ants || length(nonvsit_vert)~=0
        ...
        best_tkant = find(abs(temp_len(niter(1),:)-Lbs(niter(1)))<=10^-15); %Puede haber más de una (cogemos una cualquiera) 
        % Calculate third term of expression (7) :
        posi1 = 0;
        tau_exact(1:n,1:n) = 0;
        ...
        % Calculate second term of expression (7) :
        best_kant = find(abs(len_tour-Lbs(iter))<=10^-15); % best ant of the iteration
        best_antind2 = find(vsit_vert(best_kant(1),:));
        posi2 = 0;
        delta_best(1:sigma-1,1:n,1:n) = 0;
        for i = vsit_vert(best_kant(1),best_antind2)
            posi2 = posi2 + 1;
            if posi2 == length(best_antind2)
                break
            end
            delta_best(best_kant(1),i,vsit_vert(best_kant(1),posi2+1)) = (sigma-best_kant(1))/Lbs(iter);
            delta_best(best_kant(1),vsit_vert(best_kant(1),posi2+1),i) = delta_best(best_kant(1),i,vsit_vert(best_kant(1),posi2+1));                 
        end

        for i = 1:n
             for j = i+1:n
                 pher_mat(i,j) = rho*pher_mat(i,j) + sum(delta_best(1:sigma-1,i,j)) + sigma*tau_exact(i,j);          
                 pher_mat(j,i) = pher_mat(i,j);
             end
        end
